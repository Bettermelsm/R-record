...
######载入cellchat对象#######
###导入分组数据###
data<-subset(cellchat.use,group=="Tumor")###根据组别要修改！！！
table(data$group)
...
save(cellchat.Tumor,file = "cellchat.Tumor.Rdata")
...
#里面什么都有的
####获取所有的配受体对以及其通讯概率
df.net.Tumor <- subsetCommunication(cellchat.Tumor)
head(df.net.Tumor)

#以通路为单位提取通讯信息
df.pathway.Tumor = subsetCommunication(cellchat.Tumor,slot.name = "netP")
head(df.pathway.Tumor)

write.csv(df.net.Tumor, file='df.net.Tumor.csv',row.names = T)
write.csv(df.pathway.Tumor, file='df.pathway.Tumor.csv',row.names = T)
...
cellchat.Tumor<-netAnalysis_computeCentrality(cellchat.Tumor)
